class SchedulesController < ApplicationController
  ...
  require 'will_paginate/array'
	...
	def rewrite
		...
		if @command == "go"
			@schedule.update_attributes(project_name: params[:project_name], start_at: Time.strptime("#{params[:start_at]}", '%m/%d/%Y %I:%M %p').in_time_zone(Time.zone), end_at: (!params[:end_at].blank? ? Time.strptime("#{params[:end_at]}", '%m/%d/%Y %I:%M %p').in_time_zone(Time.zone) : nil))
